...
export const PlacePostOnlyOrder = () => {
  ...
  return useMutation({
    mutationKey: ["place-post-only-order", network, marketPubkey],
    mutationFn: async (
      {
        clientOrderId,
        maxBaseSize,
      ...
      try {
        ...
        const placePostOnlyOrderTx = await (program.methods as any)
          .placePostOnlyOrder(
            ...
            orderType, // Should be OrderType.PostOnly
